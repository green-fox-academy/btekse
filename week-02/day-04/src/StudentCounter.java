...
public class StudentCounter{
  public static void main(String... args){
    ...
    ArrayList<String> candyPeople = new ArrayList<>();

    for (int i = 0; i < map.size(); i++) {
      if ((Integer) map.get(i).get("candies") > 4) {
        candyPeople.add((String) map.get(i).get("name"));
      }
    }

    for (String name : candyPeople) {
    ...
    for (int i = 0; i < map.size(); i++) {
      if ((Integer) map.get(i).get("candies") < 5) {
        ...
        if (map.get(i).get("age") instanceof Integer) {
          sumAge += (Integer)map.get(i).get("age");
        }
      }
    }
    System.out.println("Sum of the age (students with less than 5 candy): " + sumAge);
  }
}